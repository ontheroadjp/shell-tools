...
function _wifi_check_lookup() {
    egrep "$1" | awk '{print $2}'
}

function _wifi_check_airlookup() {
    echo "${air}" | _wifi_check_lookup $1
}

function _wifi_check_okng() {
    while read line; do
...
_run() {
    _display_internet_info && _display_wifi_info && _display_radio_info
}
