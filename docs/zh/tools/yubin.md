...
首次运行时，将自动从日本邮政官网下载邮政编码 CSV。数据默认保存至 `~/.local/share/shell-tools/yubin/`（遵循 XDG Base Directory 规范）。如需自定义路径：
...
- 数据保存至 `~/.local/share/shell-tools/yubin/KEN_ALL.CSV`（若设置了 `$YUBIN_DATA_DIR` 则使用该路径）。
